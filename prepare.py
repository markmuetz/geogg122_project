...
def load_snow_hdf_data(start_date, end_date, tile='h09v05', 
	datasets=('AQUA', 'TERRA'), 
        mask_shape_file="data_files/Hydrologic_Units/HUC_Polygons.shp",
    ...
    '''
    print("  Loading datasets %s for tile %s"%(str(datasets), tile))
    ...
    cached_data_file = "%scache/load_hdf_data_%s-%s.pkl"\
	               %(settings.DATA_DIR, start_date, end_date)
	...
	target_vector_file=mask_shape_file,
	attribute_filter=2)
	...
	for dataset in datasets:
	    ...
	    if len(file_names) == 0:
		print("MISSING %s doy %i"%(dataset, doy))
...
def interp_data_over_time(masked_data, orig_mask, plot_random_values=False):
	...
	if i % 10 == 0:
	    print("    Done %2.1f percent"%(100.0 * i / masked_data.shape[1]))
...
def prepare_temperature_data(start_date, end_date, plot_graphs=False):
    ...
    f = interpolate.interp1d(x[temp_data_in_range], av_temp_data_in_range[temp_data_in_range])
